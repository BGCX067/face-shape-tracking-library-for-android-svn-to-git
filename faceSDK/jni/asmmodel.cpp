...
vector< ASMFitResult > ASMModel::fitAll(
    ...
    for (uint i=0; i<detectedObjs.size(); i++){
        ...
        // Fit it!
///__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitA0");
        ASMFitResult fitResult = fit(img(r), verbose);
///__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitA1");
        SimilarityTrans s2;
        s2.Xt = r.x;
        s2.Yt = r.y;
        s2.a = 1;
...
void ASMModel::showResult(Mat& img, const vector< ASMFitResult >& res)
{
    ...
    if (img.channels()==1)
        ...
        mb = img.clone();
///__android_log_write(ANDROID_LOG_DEBUG,"Tag","draw00");
    ShapeVec sv;
...
///__android_log_write(ANDROID_LOG_DEBUG,"Tag","draw01");
    //if (!img.empty())
        //imshow("hoho", mb);
}
...
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB0");
    curSearch.setShapeInfo( &shapeInfo );
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB1");
    curSearch.loadTrainImage(resizedImg);
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB2");
    fitResult.params = Mat_<double>::zeros(nShapeParams, 1);

    ShapeVec &sv = curSearch.shapeVec;
    ShapeVec shape_old;
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB3");
    projectParamToShape(fitResult.params, sv);
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB4");
    SimilarityTrans st = sv.getShapeTransformFitingSize(resizedImg.size(),
                                                      searchScaleRatio,
                                                      searchInitXOffset,
                                                      searchInitYOffset);
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB5");
...
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB7");
                    absSum = 0;
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB7_0");
                    for (int j=-k;j<=k;j++){
                        nrmV(j+k, 0) = img(V[j+k]);
                        absSum += fabs(nrmV(j+k, 0));
                    }
                    nrmV *= 1/absSum;
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB7_1");
                    ct = cv::Mahalanobis(nrmV, this->meanG[l][i], this->iCovarG[l][i]);
//                     printf("absSum: %lf, ct: %lf\n", absSum, ct);
////__android_log_write(ANDROID_LOG_DEBUG,"Tag","fitB8");
                    if (verbose >= ASM_FIT_VERBOSE_AT_POINT)
