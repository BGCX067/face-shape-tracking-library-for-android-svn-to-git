  ...
  Mat& img, const vector< Point >& vP, bool drawInPlace) const
{
    ...
    if (drawInPlace)
        ...
        mb = img.clone();
__android_log_write(ANDROID_LOG_DEBUG,"Tag","draw00_00");
    for (uint i=0;i<vP.size();i++){
        cv::circle(mb, vP[i], 3, cv::Scalar(25, 50, 255), 1, CV_AA);
    }
  if(vP.size() == 0){
__android_log_write(ANDROID_LOG_DEBUG,"Tag","dzero_daze");
  }
__android_log_write(ANDROID_LOG_DEBUG,"Tag","draw00_01");
  /*
    for (int i=0;i<nContours;i++){
    ...
    */
__android_log_write(ANDROID_LOG_DEBUG,"Tag","draw00_02");
    return mb;
}
